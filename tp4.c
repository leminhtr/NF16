...
int add_dir_to_sub(Dir *dir, Node *sub, Dir *father){
    ...
    if(comp>0)  //sub>dir
    {
        ...
        {
            return add_dir_to_sub(dir, sub->lc,father);
        }
    }
    ...
    if(comp<0)  //sub<dir
    {
        ...
        {
            return add_dir_to_sub(dir, sub->rc, father);
        }
...
int print_path(Dir *dir)
{
    ...
    printf("Arborescence : ");
    while(i>=0)     //affichage nom dossier de racine  current (donc ordre décroissant du tableau)
    {
